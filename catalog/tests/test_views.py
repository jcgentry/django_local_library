...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_paging(self):
        ...
        # Confirm that of the items, only 7 are displayed due to pagination.
        self.assertEqual(len(response.context['bookinstance_list']), 7)
...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        # First user can edit authors
        user_who_can_edit_authors = User.objects.create_user(username = 'testuser1', password='1X<ISRUkw+tuK')
        permission = Permission.objects.get(name='Edit author')
        user_who_can_edit_authors.user_permissions.add(permission)
        user_who_can_edit_authors.save()  

        # Second user cannot edit authors
        user_who_cannot_edit_authors = User.objects.create_user(username = 'testuser2', password='2HJ1vRV0Z&3iD')
        user_who_cannot_edit_authors.save()
    ...
    def test_default_death_date(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_form_invalid_missing_first_name(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.post(reverse('author-create'), {'last_name': 'Nolastname'})
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'form', 'first_name', 'This field is required.')

    def test_redirects_to_author_view_on_success(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.post(reverse('author-create'), {'first_name': 'Nofirstname', 'last_name': 'Nolastname'})
